...
class WanFunInpaintPipeline2(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        clip_image: list[Image.Image] = None,
        clip_neg: list[Image.Image] = None,
        max_sequence_length: int = 512,
        ...
        if video is not None:
            ...
            init_video = self.image_processor.preprocess(rearrange(video, "b c f h w -> (b f) c h w"), height=video.shape[3], width=video.shape[4]) 
        ...
        if clip_image is not None:
            clip_image_tensors = torch.stack(
                [TF.to_tensor(img).sub_(0.5).div_(0.5).to(device, weight_dtype) 
                for img in clip_image], 
                dim=0
            ) # 形状: (F, C, H, W) -> (81, 3, H, W)
            # 2. 模拟训练时的输入格式 (C, 1, H, W) 并批量处理 F 帧
            clip_context_list = []
            for single_frame_tensor in clip_image_tensors:
                # single_frame_tensor 形状: (C, H, W) -> (3, H, W)
                # 模拟训练时的输入格式: clip_image[:, None, :, :]
                # 转换为 (C, 1, H, W) -> (3, 1, H, W)
                input_to_encoder = single_frame_tensor[:, None, :, :]
                # 编码，返回 _clip_context 形状: (1, N_tokens, D_features)
                _clip_context = self.clip_image_encoder([input_to_encoder])
                # 由于推理时不需要随机 zero init，直接添加到列表中
                clip_context_list.append(_clip_context)
            # 3. 合并所有 clip 输出
            # [F, 1, N_tokens, D_features]
            clip_context_cat = torch.cat(clip_context_list, dim=0)  
            # 4. 恢复原来的 batch/time 结构 (B=1, T=F)
            # 训练时的逻辑是 clip_context_cat.view(B, T, seq_len, dim)
            B = 1 # 推理时 B=1
            T = len(clip_image) # T=81
            clip_context_final = clip_context_cat.view(B, T, clip_context_cat.size(1), clip_context_cat.size(2))
            # 5. 求平均，得到最终的 clip_context (1, N_tokens, D_features)
            # 训练时求平均是 clip_context_final.mean(dim=1)
            clip_context = clip_context_final.mean(dim=1)
        else:
        ...
        if clip_neg is not None:
            ...
            # 2. 模拟训练时的输入格式 (C, 1, H, W) 并批量处理 F 帧
            clip_context_list = []
            for single_frame_tensor in clip_neg_tensors:
                # single_frame_tensor 形状: (C, H, W) -> (3, H, W)
                # 模拟训练时的输入格式: clip_neg[:, None, :, :]
                # 转换为 (C, 1, H, W) -> (3, 1, H, W)
                input_to_encoder = single_frame_tensor[:, None, :, :]
                # 编码，返回 _clip_context 形状: (1, N_tokens, D_features)
                _clip_context = self.clip_image_encoder([input_to_encoder])
            ...
            B = 1 # 推理时 B=1
            T = len(clip_neg) # T=81
            clip_context_final_neg = clip_context_cat.view(B, T, clip_context_cat.size(1), clip_context_cat.size(2))
            # 5. 求平均，得到最终的 clip_context (1, N_tokens, D_features)
            # 训练时求平均是 clip_context_final.mean(dim=1)
            clip_context_neg = clip_context_final.mean(dim=1)
        else:
            clip_neg = Image.new("RGB", (512, 512), color=(0, 0, 0))  
            clip_neg = TF.to_tensor(clip_neg).sub_(0.5).div_(0.5).to(device, weight_dtype) 
            clip_context_neg = self.clip_image_encoder([clip_neg[:, None, :, :]])
            clip_context_neg = torch.zeros_like(clip_context_neg)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                clip_context_input = (
                    torch.cat([clip_context_neg, clip_context], dim=0) if do_classifier_free_guidance else clip_context
                )
