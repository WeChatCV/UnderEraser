...
class WanFunInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if clip_image is not None:
            ...
            B = 1
            T = clip_context_cat.size(0) 
            seq_len = clip_context_cat.size(1) 
            dim = clip_context_cat.size(2) 
            clip_context_final = clip_context_cat.view(B, T, seq_len, dim) 
            first_frame_context = clip_context_final[:, 0:1, :, :] 
            remaining_context = clip_context_final[:, 1:, :, :]   
            group_size = 4
            T_new = remaining_context.size(1) // group_size  
            averaged_context = remaining_context.view(B, T_new, group_size, seq_len, dim).mean(dim=2) 
            clip_context_new = torch.cat([first_frame_context, averaged_context], dim=1)
            clip_context = clip_context_new.squeeze(0)
