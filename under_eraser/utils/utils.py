...
def get_video_and_mask(input_video_path, video_length, sample_size, fps=None, input_mask_path=None):
    # --- 1. 处理 input_video (基于代码 1/2 的视频逐帧读取逻辑) ---
    if input_video_path is not None:
        if isinstance(input_video_path, str):
            cap = cv2.VideoCapture(input_video_path)
            input_video = []
            ...
            original_fps = cap.get(cv2.CAP_PROP_FPS)
            # 注意：代码1和代码2的frame_skip计算略有不同，这里使用代码2的更简洁版本
            ...
            while True:
                ...
                if frame_count % frame_skip == 0:
                    frame = cv2.resize(frame, (sample_size[1], sample_size[0]))
                    input_video.append(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)) # BGR to RGB (0-255 uint8)
        ...
        # 转换为 Tensor，裁剪，维度重排 (C, F, H, W)，添加 Batch 维度 (1, C, F, H, W)，并归一化 (0.0-1.0)
        input_video_tensor = torch.from_numpy(np.array(input_video))[:video_length]
        input_video_tensor = input_video_tensor.permute([3, 0, 1, 2]).unsqueeze(0) / 255.0

    else:
        input_video_tensor = None

    # --- 2. 处理 input_mask (基于代码 1/2 的视频逐帧读取逻辑) ---
    if input_mask_path is not None:
        ...
        # 二值化：< 240 -> 0 (已知/保留), >= 240 -> 1 (未知/遮挡)
        mask_bin = np.where(mask_np < 240, 0, 1).astype(np.uint8)
        
        mask_tensor = torch.from_numpy(mask_bin)        
        mask_tensor = mask_tensor.unsqueeze(1)          
        mask_tensor = mask_tensor.unsqueeze(0)          
        # input_mask 的形状: (1, 1, F, H, W)
        input_mask_tensor = mask_tensor.permute(0,2,1,3,4).float()      
    else:
        # 如果没有掩码，创建一个全遮挡（255）的掩码，但因为这里只需要用于clip_image生成，我们跳过全黑生成逻辑
        input_mask_tensor = None


    # --- 3. 生成 clip_image_list (基于代码 2 的遮罩逻辑) ---
    clip_image_list = None
    if input_video_tensor is not None and input_mask_tensor is not None:
        
        # 扩展掩码的通道维度以匹配 input_video (C=3)
        # input_mask_tensor 的形状是 (1, 1, F, H, W)
        
        # 创建一个反转的掩码（保留已知区域=1，遮挡未知区域=0）
        inverse_mask = (1.0 - input_mask_tensor) 
        inverse_mask_3ch = inverse_mask.repeat(1, 3, 1, 1, 1)

        # 遮罩后的视频 = 视频 * 反转掩码 (未知区域变为 0，即黑色)
        masked_video_tensor = input_video_tensor * inverse_mask_3ch
        
        # 将 Tensor 转换为 PIL 图像列表
        # 形状转换: (1, C, F, H, W) -> (F, C, H, W)
        frames_tensor = masked_video_tensor.squeeze(0).permute(1, 0, 2, 3) 
        clip_image_list = []
        for frame in frames_tensor:
            ...
            clip_image_list.append(Image.fromarray(frame_np))
    
    # --- 4. 返回结果 ---
    # input_video_tensor 和 input_mask_tensor 的形状和内容与代码1/2相同
    # clip_image_list 为被遮罩的 PIL 图像列表
    return input_video_tensor, input_mask_tensor, clip_image_list

def get_video_and_mask2(input_video_path, video_length, sample_size, fps=None, input_mask_path=None):
    # --- 1. 处理 input_video (基于代码 1/2 的视频逐帧读取逻辑) ---
    if input_video_path is not None:
        if isinstance(input_video_path, str):
            cap = cv2.VideoCapture(input_video_path)
    ...
    # --- 4. 生成 clip_image_list (基于代码 2 的遮罩逻辑) ---
    clip_ob_list = None
    if input_video_tensor is not None and input_mask_tensor is not None:
        ...
        # 创建一个反转的掩码（保留已知区域=1，遮挡未知区域=0）
        inverse_mask_ob = input_mask_tensor 
        inverse_mask_3ch_ob = inverse_mask_ob.repeat(1, 3, 1, 1, 1)

        # 遮罩后的视频 = 视频 * 反转掩码 (未知区域变为 0，即黑色)
        masked_video_tensor_ob = input_video_tensor * inverse_mask_3ch_ob
        
        # 将 Tensor 转换为 PIL 图像列表
        # 形状转换: (1, C, F, H, W) -> (F, C, H, W)
        frames_tensor_ob = masked_video_tensor_ob.squeeze(0).permute(1, 0, 2, 3) 
        clip_ob_list = []
        for frame in frames_tensor_ob:
            # frame 形状: (C, H, W)
            frame_np = frame.permute(1, 2, 0).cpu().numpy() # (H, W, C)
            # 转换回 0-255 范围的 uint8
            frame_np = (frame_np * 255).astype(np.uint8) 
            clip_ob_list.append(Image.fromarray(frame_np))


    # --- 4. 返回结果 ---
    # input_video_tensor 和 input_mask_tensor 的形状和内容与代码1/2相同
    # clip_image_list 为被遮罩的 PIL 图像列表
    return input_video_tensor, input_mask_tensor, clip_image_list, clip_ob_list
