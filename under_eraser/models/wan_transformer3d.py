...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0):
        ...
        original_B, original_L1, dim = x.shape
        assert original_B == 1

        T = 21
        assert original_L1 % T == 0
        Lf = original_L1 // T

        # context: [21, 257+Lt, C]
        assert context.size(0) == T

        context_img = context[:, :257, :]   # [21, 257, C]
        context_txt = context[:, 257:, :]   # [21, Lt,  C]

        # x: [1, 9408, C] -> [21, 448, C]
        x_frame = x.view(T, Lf, dim)

        b, n, d = x_frame.size(0), self.num_heads, self.head_dim

        # q from per-frame latent tokens
        q = self.norm_q(self.q(x_frame.to(dtype))).view(b, -1, n, d)   # [21, 448, H, D]

        # text cross-attn
        k_txt = self.norm_k(self.k(context_txt.to(dtype))).view(b, -1, n, d)
        v_txt = self.v(context_txt.to(dtype)).view(b, -1, n, d)
        ...
        # fuse
        out = txt_x.flatten(2) + img_x.flatten(2)   # [21, 448, C]
        out = self.o(out)

        # [21, 448, C] -> [1, 9408, C]
        out = out.reshape(1, original_L1, dim)
        return out
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        if clip_fea is not None:
            ...
            T_prime = clip_fea.size(0)  # T_prime = 21
            context_repeated = context.repeat(21, 1, 1) # [21, L, C]
            context = torch.concat([context_clip, context_repeated], dim=1)
        ...
        if self.teacache is not None:
            ...
            for idx, block in enumerate(self.blocks):#idx 从0到39
                if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                #print(x.shape) #torch.Size([1, 9408, 5120])
